	...
	<h2>배송 관리 페이지</h2>
		...
		<div class="paging">
			...
			<c:if test="${currentPage > 1}">
				<a class="first" href="<c:url value="/askdetail/managerdelivery.do?page=1" />">&lt;&lt;</a>
				<a class="prev" href="<c:url value="/askdetail/managerdelivery.do?page=${currentPage-1}" />">&lt;</a>
			</c:if>
			<c:forEach begin="${startPage}" end="${endPage > totalPage ? totalPage : endPage}" var="pageNum">
				<c:choose>
					...
					<c:otherwise>
						<a href="<c:url value="/askdetail/managerdelivery.do?page=${pageNum}" />">${pageNum}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${currentPage < totalPage}">
				<a class="next" href="<c:url value="/askdetail/managerdelivery.do?page=${currentPage+1}" />">&gt;</a>
				<a class="last" href="<c:url value="/askdetail/managerdelivery.do?page=${totalPage}" />">&gt;&gt;</a>
			</c:if>
		</div>
</body>
</html>