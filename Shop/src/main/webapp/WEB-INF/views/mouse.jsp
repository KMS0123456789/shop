	...
	<div>
		<ul id="page">
			...
			<c:if test="${currentPage > 1}">
					<li><a href="<c:url value="/peripheral/mouse.do?page=1" />">&lt;&lt;</a></li>
					<li><a href="<c:url value="/peripheral/mouse.do?page=${currentPage-1}" />">&lt;</a></li>
			</c:if>
			<c:forEach begin="${startPage}" end="${endPage > totalPage ? totalPage : endPage}" var="pageNum">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="<c:url value="/peripheral/mouse.do?page=${pageNum}" />">${pageNum}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${currentPage < totalPage}">
				<li><a href="<c:url value="/peripheral/mouse.do?page=${currentPage+1}" />">&gt;</a></li>
				<li><a href="<c:url value="/peripheral/mouse.do?page=${totalPage}" />">&gt;&gt;</a></li>
			</c:if>
		</ul>
	</div>
</body>
